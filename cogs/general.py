...
class FullCalculator(View):
    ...
    def add_digit_buttons(self):
        ...
        for label, style in digits:
            self.add_item(
                Button(label=label, style=getattr(discord.ButtonStyle, style))
            )
    ...
    def add_operator_buttons(self):
        operators = ['+', '-', '*', '/', '(', ')']
        for op in operators:
            self.add_item(Button(label=op, style=discord.ButtonStyle.primary))

    # Control buttons
    def add_control_buttons(self):
        self.add_item(Button(label="C", style=discord.ButtonStyle.danger))
        self.add_item(Button(label="⌫", style=discord.ButtonStyle.secondary))  # backspace
        self.add_item(Button(label="=", style=discord.ButtonStyle.success))

    # Handle all button clicks dynamically
    @discord.ui.button(label="dummy", style=discord.ButtonStyle.secondary, disabled=True)
    async def dummy(self, interaction: discord.Interaction, button: Button):
        pass  # placeholder to allow dynamic buttons

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        # Dynamically handle button clicks
        label = interaction.data['custom_id'] if 'custom_id' in interaction.data else None
        if not label:
            label = interaction.data['component_type']  # fallback

        # We override default Button callbacks
        return True
...
class CalcCog(commands.Cog):
    ...
    @app_commands.command(name="calc", description="Calculator command")
    @app_commands.describe(expression="Optional: math expression to evaluate")
    async def calc(self, interaction: discord.Interaction, expression: str = None):
        if expression:
            ...
            # Build full calculator view with buttons
            view = View(timeout=None)

            # Digits
            for row in [['7','8','9'], ['4','5','6'], ['1','2','3'], ['0','.']]:
                for digit in row:
                    async def click(interaction, d=digit):
                        if hasattr(view, 'expression'):
                            view.expression += d
                        else:
                            view.expression = d
                        await view.update(interaction)
                    btn = Button(label=digit, style=discord.ButtonStyle.secondary)
                    btn.callback = click
                    view.add_item(btn)

            # Operators
            for op in ['+', '-', '*', '/', '(', ')']:
                async def click(interaction, o=op):
                    if hasattr(view, 'expression'):
                        view.expression += o
                    else:
                        view.expression = o
                    await view.update(interaction)
                btn = Button(label=op, style=discord.ButtonStyle.primary)
                btn.callback = click
                view.add_item(btn)
            ...
            async def clear(interaction):
                view.expression = ""
                await view.update(interaction)
            btn_clear = Button(label="C", style=discord.ButtonStyle.danger)
            btn_clear.callback = clear
            view.add_item(btn_clear)

            async def backspace(interaction):
                if hasattr(view, 'expression') and view.expression:
                    view.expression = view.expression[:-1]
                await view.update(interaction)
            btn_back = Button(label="⌫", style=discord.ButtonStyle.secondary)
            btn_back.callback = backspace
            view.add_item(btn_back)

            async def equals(interaction):
                try:
                    result = simple_eval(getattr(view, 'expression', ''))
                    view.expression = str(result)
                except Exception:
                    view.expression = "Error"
                await view.update(interaction)
            btn_equals = Button(label="=", style=discord.ButtonStyle.success)
            btn_equals.callback = equals
            view.add_item(btn_equals)

            # Initialize expression attribute
            view.expression = ""

            # Send message
            embed = discord.Embed(title="Calculator", description="```\n \n```", color=0x00ff00)
