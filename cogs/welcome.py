import discord from discord.ext import commands from discord import app_commands import json import os import re

CONFIG_FILE = "/data/welcome_config.json" IMAGE_EXTENSIONS = (".png", ".jpg", ".jpeg", ".webp", ".gif") QUESTION_COLOR = 0xFFB700
ERROR_COLOR = 0xFF0000
SUCCESS_COLOR = 0x00FF00

def load_config(): if not os.path.exists(CONFIG_FILE): return {} try: with open(CONFIG_FILE, "r") as f: return json.load(f) except (json.JSONDecodeError, ValueError): return {}

def save_config(data): with open(CONFIG_FILE, "w") as f: json.dump(data, f, indent=4)

def format_placeholders(template: str, member: discord.Member): if not template: return "" return ( template.replace("{mention}", member.mention) .replace("{user}", str(member)) .replace("{server}", member.guild.name) .replace("{count}", str(len(member.guild.members))) )

class WelcomeLeave(commands.Cog): def init(self, bot): self.bot = bot
...
async def start_setup(self, user: discord.User, guild: discord.Guild, event_type: str):
    try:
        ...
        steps = [
            {"name":"Channel","key":"channel_id","prompt":"Provide the Channel ID.","allow_skip":False,"can_back":False,"image":False,"placeholders":None},
            {"name":"Mode","key":"mode","prompt":"Choose text or embed.","allow_skip":False,"can_back":True,"image":False,"placeholders":None},
            {"name":"Text Message","key":"text","prompt":"Enter text message.","allow_skip":True,"can_back":True,"image":False,"placeholders":"{mention}, {user}, {server}, {count}"},
            {"name":"Text Image","key":"image_url","prompt":"Text image URL or attachment.","allow_skip":True,"can_back":True,"image":True,"placeholders":"{member_pfp}, {server_icon}"},
            {"name":"Embed Title","key":"title","prompt":"Embed title.","allow_skip":False,"can_back":True,"image":False,"placeholders":"{mention}, {user}, {server}, {count}"},
            {"name":"Embed Description","key":"description","prompt":"Embed description.","allow_skip":True,"can_back":True,"image":False,"placeholders":"{mention}, {user}, {server}, {count}"},
            {"name":"Embed Color","key":"color","prompt":"HEX color like #00ff00.","allow_skip":True,"can_back":True,"image":False,"placeholders":None},
            {"name":"Embed Image","key":"image_url","prompt":"Embed image URL.","allow_skip":True,"can_back":True,"image":True,"placeholders":"{member_pfp}, {server_icon}"},
            {"name":"Embed Thumbnail","key":"thumbnail_url","prompt":"Embed thumbnail URL.","allow_skip":True,"can_back":True,"image":True,"placeholders":"{member_pfp}, {server_icon}"},
            {"name":"Embed Author Icon","key":"icon_url","prompt":"Embed author icon URL.","allow_skip":True,"can_back":True,"image":True,"placeholders":"{member_pfp}, {server_icon}"},
            {"name":"Embed Footer Text","key":"footer_text","prompt":"Embed footer text.","allow_skip":True,"can_back":True,"image":False,"placeholders":"{mention}, {user}, {server}, {count}"},
            {"name":"Embed Footer Icon","key":"footer_icon","prompt":"Embed footer icon URL.","allow_skip":True,"can_back":True,"image":True,"placeholders":"{member_pfp}, {server_icon}"}
        ]
        ...
        while index < len(steps):
            ...
            if step["key"] not in ("text","image_url") and step["key"] not in ("title","description","color","thumbnail_url","icon_url","footer_text","footer_icon") and data.get("mode") == "text" and step["key"] not in ("channel_id","mode"):
                index += 1
                continue
            answer = await self.ask_input(dm, user, step["name"], step["prompt"], allow_skip=step["allow_skip"], image=step["image"], placeholders=step.get("placeholders"))
            if answer == "back":
            ...
            if step["key"] == "channel_id":
                ...
                if mode not in ["text","embed"]:
                    await dm.send(embed=discord.Embed(description="❌ Invalid mode.", color=ERROR_COLOR))
                    continue
                answer = mode
...
async def setup(bot): await bot.add_cog(WelcomeLeave(bot))
