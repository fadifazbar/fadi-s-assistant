...
class Moderation(commands.Cog):
    ...
    # --- SLASH COMMAND ---
    @app_commands.command(name="roleall", description="Give or remove a role from everyone")
    @app_commands.describe(
        action="Choose whether to give or remove the role",
        role="The role you want to give/remove from all members"
    )
    ...
    async def roleall_slash(self, interaction: discord.Interaction, action: app_commands.Choice[str], role: discord.Role):
        await self._handle_roleall(interaction, action.value, role, is_slash=True)
    ...
    async def roleall_prefix(self, ctx, action: str, role: discord.Role):
        ...
        if action not in ["give", "remove"]:
            return await ctx.send("⚠️ Usage: `$roleall give/remove @role`")

        await self._handle_roleall(ctx, action, role, is_slash=False)

    # --- SHARED HANDLER ---
    async def _handle_roleall(self, source, action, role, is_slash: bool):
        guild = source.guild

        # Bot permission check
        if not guild.me.guild_permissions.manage_roles:
            msg = "⚠️ I don't have permission to manage roles."
            return await (source.response.send_message(msg, ephemeral=True) if is_slash else source.send(msg))

        # Role hierarchy check
        if role >= guild.me.top_role:
            msg = "⚠️ That role is higher than or equal to my top role. I can't manage it."
            return await (source.response.send_message(msg, ephemeral=True) if is_slash else source.send(msg))

        # Build embed (role shown as plain text instead of mention)
        embed = discord.Embed(
            title=f"{'Giving' if action == 'give' else 'Removing'} Role",
            description=(
                f"{'Giving' if action == 'give' else 'Removing'} **{role.name}** "
                f"for {len(guild.members)} members!\n\n"
                f"⚠️ Note: For large servers, the bot might hit the rate limit."
            ),
            color=discord.Color.blurple()
        )
        embed.set_footer(text=f"Requested by {source.user if is_slash else source.author}")
        ...
        for member in guild.members:
            try:
                if action == "give" and role not in member.roles:
                    await member.add_roles(role, reason=f"Roleall {action} by {source.user if is_slash else source.author}")
                    success += 1
                elif action == "remove" and role in member.roles:
                    await member.remove_roles(role, reason=f"Roleall {action} by {source.user if is_slash else source.author}")
                    success += 1
            except:
                failed += 1

        # Final status embed (no role mention, just name)
        result = discord.Embed(
            title="✅ Finished!",
            description=(
                f"{'Gave' if action == 'give' else 'Removed'} **{role.name}**\n\n"
                f"✔️ Success: {success}\n"
                f"❌ Failed: {failed}"
            ),
        ...
        if is_slash:
            await source.followup.send(embed=result)
        else:
            await source.send(embed=result)
