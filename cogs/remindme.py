...
def load_reminders(): if os.path.exists(DATA_FILE): with open(DATA_FILE, "r") as f: return json.load(f) return []

def save_reminders(reminders): with open(DATA_FILE, "w") as f: json.dump(reminders, f)
...
def parse_time(timestr: str | None): if not timestr: return 0
...
class ReminderCog(commands.Cog): def init(self, bot): self.bot = bot self.reminders = load_reminders() self.active_loops = {}  # user_id -> loop control self.check_reminders.start()
...
async def setup(bot): await bot.add_cog(ReminderCog(bot))
